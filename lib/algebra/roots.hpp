...
namespace roots
{
    ...
    std::vector<R> polynomial_roots_companion_matrix(const std::vector<R>& a,R _radix_base=2,R _p_norm=2,double _confidence=0.95,I _max_itr=100)
    {
        I n = (I)a.size();
        linalg::matrix<R,I> companion_matrix(n-1,n-1);
        ...
        linalg::balancing_parlett_reinsch(companion_matrix,_radix_base,_p_norm,_confidence,_max_itr);
